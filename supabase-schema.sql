...
create table if not exists public.queue_requests (
  ...
  request_type text not null default 'catalog' check (request_type in ('catalog', 'write_in')),
  write_in_title text not null default '',
  write_in_artist text not null default '',
  ...
  payment_amount numeric(6, 2) not null default 7.00,
